define(function (require, exports, module) {
     ...
     var helpapplyedit = require("helpapplyedit");
     Dialog = require("dialog");
     
     
     //模块函数
     exports.applyFunc=function(){
    	mdboss.fillPage(require("text!template/page/help/feedback/index.html"));
		new mdboss.View.applyView();
     };
     
     mdboss.View.applyView = Backbone.View.extend({
...
//        	 'change #module_type':"search",
         ...
         modulebatch:function(){//指修改
        	 ...
        	 if(tids.length >0){
        		 var v_this = this;
        		 helpapplyedit.helpapplyeditFunc({"tids":tids,"title":"batch","renderData":v_this.renderDataBack,"pview":v_this});
        	 }else{
         ...
         dataview:function(e){
        	 var tid=$(e.currentTarget).data("tid"); 
        	 helpapplyedit.helpapplyeditFunc({"tid":tid, "title":"view","view":true});
         },
         moduleadd:function(){//新增
        	 var v_this = this;
        	 helpapplyedit.helpapplyeditFunc({"title":"add","renderData":v_this.renderDataBack,"pview":v_this});
 
         },
         dataedit:function(e){//新增修改记录
        	 var v_this = this;
        	var tid=$(e.currentTarget).data("tid"); 
        	helpapplyedit.helpapplyeditFunc({"tid":tid,"title":"edit","renderData":v_this.renderDataBack,"pview":v_this});
         },
         ...
         renderData:function(f){//渲染列表
             ...
             commonModel.save({ "usercode":mdboss.usercode,"pageNo":this.pageNo, "pageSize":v_this.pageSize,    // 每页10条记录
     			"queryCondition": v_this.condition,"orderBy":"parentcode,moduleorder"},{url:mdboss.api.sysmodulelist,success:function(model,res){
     				$("#submit-loading").hide();
     				if(res.rc == 1){
