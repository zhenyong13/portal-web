define(function (require, exports, module) {
     ...
     var assestsedit = require("assestsedit");
     Dialog = require("dialog");
     
     
     //使用帮助 
     exports.assestsFunc=function(){
    	mdboss.fillPage(require("text!template/page/fund/assests/index.html"));
		new mdboss.View.assestsView();
     };
     
     mdboss.View.assestsView = Backbone.View.extend({
         ...
         modulebatch:function(){//指修改
        	 ...
        	 if(tids.length >0){
        		 var v_this = this;
        		 assestsedit.assestseditFunc({"tids":tids,"title":"batch","renderData":v_this.renderDataBack,"pview":v_this});
        	 }else{
        	 ...
        	 mdboss.confirm("确定要删除吗？",function(c){
        		 if(c){
        			 commonModel.save({"usercode":mdboss.usercode,"tid":tid},
        					 {url:mdboss.api.assestsdelete,success:function(model,res){
             		 	if(res.rc==1){
...
// 			var v_this = this;
        	...
        	assestsedit.assestseditFunc({"tid":tid, "title":"view","view":true});
//        	 
         },
         moduleadd:function(){//新增 
        	 var v_this = this;
        	 assestsedit.assestseditFunc({"title":"add","renderData":v_this.renderDataBack,"pview":v_this});
         },
         ...
         dataedit:function(e){//新增修改记录  
        	...
        	var tid=$(e.currentTarget).data("tid"); 
        	$(e.currentTarget).closest("li").addClass("selected");//
        	assestsedit.assestseditFunc({"tid":tid,"title":"edit","renderData":v_this.renderDataBack,"pview":v_this});
         },
            	 ...
            	 mdboss.confirm("确定要删除吗？",function(c){
            		 if(c){
            			 commonModel.save({"usercode":mdboss.usercode,"tids":delIds},
            					 {url:mdboss.api.assestsdeletebatch,success:function(model,res){
                    		 if(res.rc==1){
                    			 mdboss.notifyTips("删除成功！");
                    			 v_this.renderData();
                    		 }else{
...
//        	 mdboss.dataselect(this, "flevel", "OA_ISSUELEVEL","","150");
         ...
         renderData:function(f){//渲染列表
        	 ...
        	 var tmpCondition = new Array();
	            ...
	            if(this.modulename){
	            	tmpCondition.push({"key":"serialno", "operate":"Like","v1":this.modulename});
	            }
	            if(this.modulecode){
	            	tmpCondition.push({"key":"assestname", "operate":"Like","v1":this.modulecode});
	            }
	            if(v_this.$el.find("#dtype").val()){
	            	tmpCondition.push({"key":"assesttype", "operate":"EqualTo","v1":v_this.$el.find("#dtype").val()});
	            }
             ...
             commonModel.save({ "usercode":mdboss.usercode,"pageNo":this.pageNo, "pageSize":v_this.pageSize,    // 每页10条记录
     			"queryCondition": v_this.condition,"orderBy":this.orderBy},{url:mdboss.api.assestslist,success:function(model,res){
     				$("#submit-loading").hide();
     				if(res.rc == 1){
