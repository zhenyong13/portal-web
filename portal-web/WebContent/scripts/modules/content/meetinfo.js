define(function (require, exports, module) {
     ...
     var meetinfoedit = require("meetinfoedit");
     Dialog = require("dialog");
     
     //公告
     exports.meetinfoFunc=function(){
    	mdboss.fillPage(require("text!template/page/content/meetinfo/index.html"));
		new mdboss.View.meetinfoView();
     };
     
     mdboss.View.meetinfoView = Backbone.View.extend({
        ...
        sorting:function(e){
         ...
         modulebatch:function(){//批量发布
        	 ...
        	 mdboss.confirm("确定要发布吗?", function(e){
        		 if(e){
        			 mdboss.reqsubmit({"tids":tids}, mdboss.api.meetinfobatchpublish, function(m, res){
                		 if(res.rc==1){
                			 mdboss.alert("发布成功!");
                			 t_this.renderData();
                		 }else{
         ...
         topshowNews:function(e){//置顶-
        	 ...
        	 mdboss.reqsubmit(reqData, mdboss.api.meetinfoedit, function(m, res){
        		 if(res.rc==1){
        			 mdboss.alert(msg);
        			 t_this.renderData();
        		 }else{
        	 ...
        	 mdboss.confirm("确定要删除吗？",function(c){
        		 if(c){
        			 commonModel.save({"usercode":mdboss.usercode,"tid":tid},
        					 {url:mdboss.api.meetinfodelete,success:function(model,res){
             		 	if(res.rc==1){
...
// 			var v_this = this;
        	...
        	meetinfoedit.meetinfoeditFunc({"tid":tid, "title":"view","view":true}); 
         },
         moduleadd:function(){//新增 
        	 var v_this = this;
        	 meetinfoedit.meetinfoeditFunc({"title":"add","renderData":v_this.renderDataBack,"pview":v_this}); 
         },
         ...
         dataedit:function(e){//新增修改记录  
        	...
        	var tid=$(e.currentTarget).data("tid"); 
        	$(e.currentTarget).closest("li").addClass("selected");//
        	meetinfoedit.meetinfoeditFunc({"tid":tid,"title":"edit","renderData":v_this.renderDataBack,"pview":v_this}); 
         },
            	 ...
            	 mdboss.confirm("确定要删除吗？",function(c){
            		 if(c){
            			 commonModel.save({"usercode":mdboss.usercode,"tids":delIds},
            					 {url:mdboss.api.meetinfodeletebatch,success:function(model,res){
                    		 if(res.rc==1){
                    			 mdboss.notifyTips("删除成功！");
                    			 v_this.renderData();
                    		 }else{
         ...
         renderData:function(f){//渲染列表
        	 ...
        	 var tmpCondition = new Array();
	            if(this.$el.find("#creator").attr("data-code")){
	            	tmpCondition.push({"key":"booker","operate":"EqualTo","v1":this.$el.find("#creator").attr("data-code")});
	            }
	            if(this.modulename){
	            	tmpCondition.push({"key":"meetname", "operate":"Like","v1":this.modulename});
	            }
             ...
             commonModel.save({ "usercode":mdboss.usercode,"pageNo":this.pageNo, "pageSize":v_this.pageSize,    // 每页10条记录
     			"queryCondition": v_this.condition,"orderBy":this.orderBy},{url:mdboss.api.meetinfolist,success:function(model,res){
     				$("#submit-loading").hide();
     				if(res.rc == 1){
