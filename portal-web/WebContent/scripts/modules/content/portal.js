define(function (require, exports, module) {
     ...
     //模块函数
     exports.portalFun=function(){
    	mdboss.fillPage(require("text!template/page/content/portal/index.html"));
		new mdboss.View.portalModule();
     };
     
     mdboss.View.portalModule = Backbone.View.extend({
         ...
         modulebatch:function(){//指修改
        	 ...
        	 if(tids.length >0){
        		 var v_this = this;
        		 seajs.use("portaledit", function () {
        			 new  mdboss.View.portaledit({"tids":tids,"title":"批量修改","renderData":v_this.renderDataBack,"pview":v_this});
        		 });
        	 }else{
        	 ...
        	 mdboss.confirm("确定要删除吗？",function(c){
        		 if(c){
        			 commonModel.save({"usercode":mdboss.usercode,"tid":tid},
        					 {url:mdboss.api.portaldelete,success:function(model,res){
             		 	if(res.rc==1){
         ...
         dataview:function(e){
        	 var tid=$(e.currentTarget).data("tid"); 
        	 seajs.use("portaledit", function () {
        		 new  mdboss.View.portaledit({"tid":tid, "title":"模块详情","view":true});
        	 });
         },
         moduleadd:function(){//新增
        	 var v_this = this;
        	 seajs.use("portaledit", function () {
        		 new  mdboss.View.portaledit({"title":"新增模块","renderData":v_this.renderDataBack,"pview":v_this});
        	 });
         },
         dataedit:function(e){//新增修改记录
        	...
        	var tid=$(e.currentTarget).data("tid"); 
        	 seajs.use("portaledit", function () {
        		 new  mdboss.View.portaledit({"tid":tid,"title":"编辑模块","renderData":v_this.renderDataBack,"pview":v_this});
        	 });
         },
            	 ...
            	 mdboss.confirm("确定要删除吗？",function(c){
            		 if(c){
            			 commonModel.save({"usercode":mdboss.usercode,"tids":delIds},
            					 {url:mdboss.api.portaldeletebatch,success:function(model,res){
                    		 if(res.rc==1){
                    			 mdboss.notifyTips("删除成功！");
                    			 v_this.renderData();
                    		 }else{
         ...
         renderData:function(){//渲染列表
        	 ...
        	 var tmpCondition = new Array();
	            ...
	            if(v_this.$el.find("#module_name").val()){
	            	tmpCondition.push({"key":"columnname", "operate":"Like","v1":v_this.$el.find("#module_name").val()});
	            }
	            if(v_this.$el.find("#module_type").val()){
	            	tmpCondition.push({"key":"sid", "operate":"EqualTo","v1":parseInt(v_this.$el.find("#module_type").val())});
	            }
             ...
             commonModel.save({ "usercode":mdboss.usercode,
            	 ...
            	 "pageSize":v_this.pageSize,
     			 ...
     			 "orderBy":this.orderBy},{url:mdboss.api.portalcontentsearch,success:function(model,res){
     				$("#submit-loading").hide();
     				if(res.rc == 1){
