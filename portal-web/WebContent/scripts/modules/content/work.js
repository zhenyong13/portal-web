define(function (require, exports, module) {
     ...
     var workedit = require("workedit");
     Dialog = require("dialog");
     //快捷方式
     exports.workFun=function(){
    	mdboss.fillPage(require("text!template/page/content/work/index.html"));
		new mdboss.View.workView();
     };
     
     mdboss.View.workView = Backbone.View.extend({
         ...
         modulebatch:function(){//指修改
        	 ...
        	 if(tids.length >0){
        		 var v_this = this;
        		 workedit.workeditFunc({"tids":tids,"title":"batch","renderData":v_this.renderDataBack,"pview":v_this});
        	 }else{
        	 ...
        	 mdboss.confirm("确定要删除吗？",function(c){
        		 if(c){
        			 commonModel.save({"usercode":mdboss.usercode,"tid":tid},
        					 {url:mdboss.api.shortcutdelete,success:function(model,res){
             		 	if(res.rc==1){
...
// 			var v_this = this;
        	...
        	workedit.workeditFunc({"tid":tid, "title":"view","view":true});
//        	 
         },
         moduleadd:function(){//新增 
        	 var v_this = this;
        	 console.log();
        	 workedit.workeditFunc({"title":"add","renderData":v_this.renderDataBack,"pview":v_this});
         },
         ...
         dataedit:function(e){//新增修改记录  
        	...
        	var tid=$(e.currentTarget).data("tid"); 
        	$(e.currentTarget).closest("li").addClass("selected");//
        	workedit.workeditFunc({"tid":tid,"title":"edit","renderData":v_this.renderDataBack,"pview":v_this});
         },
            	 ...
            	 mdboss.confirm("确定要删除吗？",function(c){
            		 if(c){
            			 commonModel.save({"usercode":mdboss.usercode,"tids":delIds},
            					 {url:mdboss.api.shortcutdeletebatch,success:function(model,res){
                    		 if(res.rc==1){
                    			 mdboss.notifyTips("删除成功！");
                    			 v_this.renderData();
                    		 }else{
...
//        	 mdboss.dataselect(this, "flevel", "OA_ISSUELEVEL","","150");
         ...
         renderData:function(f){//渲染列表
        	 ...
        	 var tmpCondition = new Array();
	            ...
	            if(v_this.parentcode){
	            	tmpCondition.push({"key":"orgcode", "operate":"EqualTo","v1":v_this.parentcode});
	            }
	            
	         v_this.condition = tmpCondition;
        	 $("#submit-loading").show();
        	 $("#module-flow-list").mCustomScrollbar("destroy");
        	 v_this.condition.push({"key":"usercode","operate":"IsNull","v1":""});
             commonModel.save({ "usercode":mdboss.usercode,"pageNo":this.pageNo, "pageSize":v_this.pageSize,    // 每页10条记录
     			"queryCondition": v_this.condition,"orderBy":this.orderBy},{url:mdboss.api.shortcutlist,success:function(model,res){
     				$("#submit-loading").hide();
     				if(res.rc == 1){
     	            	 var listTemplate = Handlebars.compile(v_this.module_list_html);
     	            	 
     	            	 var listHtml=listTemplate({data:res.data,"baseurl": baseConfig.avatar_url});
