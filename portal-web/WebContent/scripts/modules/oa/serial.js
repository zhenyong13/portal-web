define(function (require, exports, module) {
     ...
     var serialedit = require("serialedit");
     Dialog = require("dialog");
     
     
     //使用帮助 
     exports.serialFunc=function(){
    	mdboss.fillPage(require("text!template/page/oa/serial/index.html"));
		new mdboss.View.serialView();
     };
     
     mdboss.View.serialView = Backbone.View.extend({
         ...
         events: {
        	 ...
        	 'change #module_type':"filterSearch",
         ...
         modulebatch:function(){//指修改
        	 ...
        	 if(tids.length >0){
        		 var v_this = this;
        		 serialedit.serialeditFunc({"tids":tids,"title":"batch","renderData":v_this.renderDataBack,"pview":v_this});
        	 }else{
        	 ...
        	 mdboss.confirm("确定要删除吗？",function(c){
        		 if(c){
        			 commonModel.save({"usercode":mdboss.usercode,"tid":tid},
        					 {url:mdboss.api.serialdelete,success:function(model,res){
             		 	if(res.rc==1){
...
// 			var v_this = this;
        	...
        	serialedit.serialeditFunc({"tid":tid, "title":"view","view":true});
//        	 
         },
         moduleadd:function(){//新增 
        	 var v_this = this;
        	 serialedit.serialeditFunc({"title":"add","renderData":v_this.renderDataBack,"pview":v_this});
         },
         ...
         dataedit:function(e){//新增修改记录  
        	...
        	var tid=$(e.currentTarget).data("tid"); 
        	$(e.currentTarget).closest("li").addClass("selected");//
        	serialedit.serialeditFunc({"tid":tid,"title":"edit","renderData":v_this.renderDataBack,"pview":v_this});
         },
            	 ...
            	 mdboss.confirm("确定要删除吗？",function(c){
            		 if(c){
            			 commonModel.save({"usercode":mdboss.usercode,"tids":delIds},
            					 {url:mdboss.api.serialdeletebatch,success:function(model,res){
                    		 if(res.rc==1){
                    			 mdboss.notifyTips("删除成功！");
                    			 v_this.renderData();
                    		 }else{
         ...
         render:function(){
        	 mdboss.dataselect(this, "module_type", "CM_ASSETTYPE","","150");
        	 this.renderData( );
         },
         renderData:function(){//渲染列表
        	 ...
        	 var tmpCondition = new Array();
	            ...
	            if(this.modulename){
	            	tmpCondition.push({"key":"serialcode", "operate":"Like","v1":this.modulename});
	            }
	            if(this.parentcode){
	            	tmpCondition.push({"key":"orgcode", "operate":"EqualTo","v1":this.parentcode});
	            }
	            if(this.modulecode){
	            	tmpCondition.push({"key":"year", "operate":"Like","v1":this.modulecode});
	            }
	            if(v_this.$el.find("#module_type").val()){
	            	tmpCondition.push({"key":"type", "operate":"EqualTo","v1":v_this.$el.find("#module_type").val()});
	            }
             ...
             commonModel.save({ "usercode":mdboss.usercode,"pageNo":this.pageNo, "pageSize":v_this.pageSize,    // 每页10条记录
     			"queryCondition": v_this.condition,"orderBy":this.orderBy},{url:mdboss.api.seriallist,success:function(model,res){
     				$("#submit-loading").hide();
     				if(res.rc == 1){
