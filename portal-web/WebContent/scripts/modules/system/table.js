define(function (require, exports, module) {
     ...
     //库表函数
     exports.tablemoduelFun=function(){
    	mdboss.fillPage(require("text!template/page/system/table/index.html"));
		new mdboss.View.tableModule();
     };
     
     mdboss.View.tableModule = Backbone.View.extend({
         ...
         events: {
        	 ...
        	 'keyup #table_name':"filterSearch",//搜索
        	 'keyup #table_cname':"filterSearch",//搜索
        	 'keyup #db_name':"filterSearch",//搜索
        	 'keyup #db_cname':"filterSearch",//搜索
        	 'keyup #app_code':"filterSearch",//搜索
        	 'click #module-flow-list li span.js-check-box i':'checkbox',//checkobx 选中取消
        	 'click .js_search_module':'searchModule',//库表搜索
         ...
         modulebatch:function(){//指修改
        	 ...
        	 if(tids.length >0){
        		 var v_this = this;
        		 seajs.use("tableedit", function () {
        			 new  mdboss.View.tableedit({"tids":tids,"title":"批量修改","renderData":v_this.renderDataBack,"pview":v_this});
        		 });
        	 }else{
         ...
         search:function(treeNode, parent, pview){//  查询父库表      	
        	 pview.pageNo = 1;
        	 ...
        	 mdboss.confirm("确定要删除吗？",function(c){
        		 if(c){
        			 commonModel.save({"usercode":mdboss.usercode,"tid":tid},
        					 {url:mdboss.api.tabledelete,success:function(model,res){
             		 	if(res.rc==1){
         ...
         dataview:function(e){
        	 var tid=$(e.currentTarget).data("tid"); 
        	 seajs.use("tableedit", function () {
        		 new  mdboss.View.tableedit({"tid":tid, "title":"库表详情","view":true});
        	 });
         },
         moduleadd:function(){//新增
        	 var v_this = this;
        	 seajs.use("tableedit", function () {
        		 new  mdboss.View.tableedit({"title":"新增库表","renderData":v_this.renderDataBack,"pview":v_this});
        	 });
         },
         dataedit:function(e){//新增修改记录
        	...
        	var tid=$(e.currentTarget).data("tid"); 
        	 seajs.use("tableedit", function () {
        		 new  mdboss.View.tableedit({"tid":tid,"title":"编辑库表","renderData":v_this.renderDataBack,"pview":v_this});
        	 });
         },
            	 ...
            	 mdboss.confirm("确定要删除吗？",function(c){
            		 if(c){
            			 commonModel.save({"usercode":mdboss.usercode,"tids":delIds},
            					 {url:mdboss.api.tabledeletebatch,success:function(model,res){
                    		 if(res.rc==1){
                    			 mdboss.notifyTips("删除成功！");
                    			 v_this.renderData();
                    		 }else{
         ...
         renderData:function(f){//渲染列表
        	 ...
        	 var tmpCondition = new Array();
	            if(v_this.$el.find("#db_name").val()){
	            	tmpCondition.push({"key":"dbname","operate":"Like","v1":v_this.$el.find("#db_name").val()});
	            }
	            if(v_this.$el.find("#db_cname").val()){
	            	tmpCondition.push({"key":"dbcname","operate":"Like","v1":v_this.$el.find("#db_cname").val()});
	            }
	            if(v_this.$el.find("#table_name").val()){
	            	tmpCondition.push({"key":"tablename","operate":"Like","v1":v_this.$el.find("#table_name").val()});
	            }
	            if(v_this.$el.find("#table_cname").val()){
	            	tmpCondition.push({"key":"tablecname","operate":"Like","v1":v_this.$el.find("#table_cname").val()});
	            }
	            if(v_this.$el.find("#app_code").val()){
	            	tmpCondition.push({"key":"appcode","operate":"Like","v1":v_this.$el.find("#app_code").val()});
	            }
             ...
             commonModel.save({ "usercode":mdboss.usercode,"pageNo":this.pageNo, "pageSize":v_this.pageSize,    // 每页10条记录
     			"queryCondition": v_this.condition,"orderBy":this.orderBy},{url:mdboss.api.tablelist,success:function(model,res){
     				$("#submit-loading").hide();
     				if(res.rc == 1){
