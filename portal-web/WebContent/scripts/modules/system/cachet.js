define(function (require, exports, module) {
     ...
     var cachetedit = require("cachetedit");
     Dialog = require("dialog");
     
     
     //使用帮助 
     exports.cachetFun=function(){
    	mdboss.fillPage(require("text!template/page/system/cachet/index.html"));
		new mdboss.View.cachetView();
     };
     
     mdboss.View.cachetView = Backbone.View.extend({
         ...
         modulebatch:function(){//指修改
        	 ...
        	 if(tids.length >0){
        		 var v_this = this;
        		 cachetedit.cacheteditFunc({"tids":tids,"title":"batch","renderData":v_this.renderDataBack,"pview":v_this});
        	 }else{
        	 ...
        	 mdboss.confirm("确定要删除吗？",function(c){
        		 if(c){
        			 commonModel.save({"usercode":mdboss.usercode,"tid":tid},
        					 {url:mdboss.api.cachetdelete,success:function(model,res){
             		 	if(res.rc==1){
...
// 			var v_this = this;
        	...
        	cachetedit.cacheteditFunc({"tid":tid, "title":"view","view":true});
//        	 
         },
         moduleadd:function(){//新增 
        	 var v_this = this;
        	 cachetedit.cacheteditFunc({"title":"add","renderData":v_this.renderDataBack,"pview":v_this});
         },
         ...
         dataedit:function(e){//新增修改记录  
        	...
        	var tid=$(e.currentTarget).data("tid"); 
        	$(e.currentTarget).closest("li").addClass("selected");//
        	cachetedit.cacheteditFunc({"tid":tid,"title":"edit","renderData":v_this.renderDataBack,"pview":v_this});
         },
            	 ...
            	 mdboss.confirm("确定要删除吗？",function(c){
            		 if(c){
            			 commonModel.save({"usercode":mdboss.usercode,"tids":delIds},
            					 {url:mdboss.api.cachetdeletebatch,success:function(model,res){
                    		 if(res.rc==1){
                    			 mdboss.notifyTips("删除成功！");
                    			 v_this.renderData();
                    		 }else{
...
//        	 mdboss.dataselect(this, "flevel", "OA_ISSUELEVEL","","150");
         ...
         renderData:function(f){//渲染列表
        	 ...
        	 var tmpCondition = new Array();
	            ...
	            if(this.modulename){
	            	tmpCondition.push({"key":"cachetname", "operate":"Like","v1":this.modulename});
	            }
             ...
             commonModel.save({ "usercode":mdboss.usercode,"pageNo":this.pageNo, "pageSize":v_this.pageSize,    // 每页10条记录
     			"queryCondition": v_this.condition,"orderBy":this.orderBy},{url:mdboss.api.cachetlist,success:function(model,res){
     				$("#submit-loading").hide();
     				if(res.rc == 1){
