define(function (require, exports, module) {
     ...
     //库表字段函数
     exports.columnmoduelFun=function(){
    	mdboss.fillPage(require("text!template/page/system/column/index.html"));
		new mdboss.View.columnModule();
     };
     
     mdboss.View.columnModule = Backbone.View.extend({
         ...
         events: {
        	 ...
        	 'change #db_name':"search",
        	 'change #table_name':"search",
        	 'keyup #column_name':"filterSearch",//搜索
        	 'keyup #column_cname':"filterSearch",//搜索
        	 'click #module-flow-list li span.js-check-box i':'checkbox',//checkobx 选中取消
        	 'click .js_search_module':'searchModule',//库表字段搜索
         ...
         modulebatch:function(){//指修改
        	 ...
        	 if(tids.length >0){
        		 var v_this = this;
        		 seajs.use("columnedit", function () {
        			 new  mdboss.View.columnedit({"tids":tids,"title":"批量修改","renderData":v_this.renderDataBack,"pview":v_this});
        		 });
        	 }else{
        	 ...
        	 mdboss.confirm("确定要删除吗？",function(c){
        		 if(c){
        			 commonModel.save({"usercode":mdboss.usercode,"tid":tid},
        					 {url:mdboss.api.columndelete,success:function(model,res){
             		 	if(res.rc==1){
         ...
         dataview:function(e){
        	 var tid=$(e.currentTarget).data("tid"); 
        	 seajs.use("columnedit", function () {
        		 new  mdboss.View.columnedit({"tid":tid, "title":"库表字段详情","view":true});
        	 });
         },
         moduleadd:function(){//新增
        	 var v_this = this;
        	 seajs.use("columnedit", function () {
        		 new  mdboss.View.columnedit({"title":"新增库表字段","renderData":v_this.renderDataBack,"pview":v_this});
        	 });
         },
         dataedit:function(e){//新增修改记录
        	...
        	var tid=$(e.currentTarget).data("tid"); 
        	 seajs.use("columnedit", function () {
        		 new  mdboss.View.columnedit({"tid":tid,"title":"编辑库表字段","renderData":v_this.renderDataBack,"pview":v_this});
        	 });
         },
            	 ...
            	 mdboss.confirm("确定要删除吗？",function(c){
            		 if(c){
            			 commonModel.save({"usercode":mdboss.usercode,"tids":delIds},
            					 {url:mdboss.api.columndeletebatch,success:function(model,res){
                    		 if(res.rc==1){
                    			 mdboss.notifyTips("删除成功！");
                    			 v_this.renderData();
                    		 }else{
         ...
         renderData:function(f){//渲染列表
        	 ...
        	 if(v_this.$el.find("#db_name").val()){
	            	tmpCondition.push({"key":"dbname","operate":"EqualTo","v1":v_this.$el.find("#db_name").val()});
	         }
        	 if(v_this.$el.find("#table_name").val()){
        		 tmpCondition.push({"key":"tablename","operate":"EqualTo","v1":v_this.$el.find("#table_name").val()});
        	 }
        	 if(v_this.$el.find("#column_name").val()){
        		 tmpCondition.push({"key":"columnname","operate":"Like","v1":v_this.$el.find("#column_name").val()});
        	 }
        	 if(v_this.$el.find("#column_cname").val()){
        		 tmpCondition.push({"key":"columncname","operate":"Like","v1":v_this.$el.find("#column_cname").val()});
        	 }
             ...
             commonModel.save({ "usercode":mdboss.usercode,"pageNo":this.pageNo, "pageSize":v_this.pageSize,    // 每页10条记录
     			"queryCondition": v_this.condition,"orderBy":this.orderBy},{url:mdboss.api.columnlist,success:function(model,res){
     				$("#submit-loading").hide();
     				if(res.rc == 1){
