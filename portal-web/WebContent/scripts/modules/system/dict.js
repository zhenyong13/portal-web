...
define(function (require, exports, module) {
     ...
     mdboss.View.systemDict = Backbone.View.extend({
        	 ...
        	 mdboss.confirm("确定要删除吗？",function(c){
        		 if(c){
        			 commonModel.save({"usercode":mdboss.usercode,"tid":tid},
        					 {url:mdboss.api.dictdelete,success:function(model,res){
             		 	if(res.rc==1){
         ...
         dataview:function(e){
        	 var tid=$(e.currentTarget).data("tid"); 
        	 seajs.use("dictedit", function () {
        		 new  mdboss.View.dictedit({"tid":tid, "title":"数据字典详情","view":true});
        	 });
         },
         moduleadd:function(){//新增
        	 var v_this = this;
        	 seajs.use("dictedit", function () {
        		 new  mdboss.View.dictedit({"title":"新增数据字典","renderData":v_this.renderDataBack,"pview":v_this});
        	 });
         },
         modulebatch:function(){//指修改
        	 ...
        	 if(tids.length >0){
        		 var v_this = this;
        		 seajs.use("dictedit", function () {
        			 new  mdboss.View.dictedit({"tids":tids,"title":"批量修改","renderData":v_this.renderDataBack,"pview":v_this});
        		 });
        	 }else{
         ...
         dataedit:function(e){//新增修改记录
        	...
        	var tid=$(e.currentTarget).data("tid"); 
        	 seajs.use("dictedit", function () {
        		 new  mdboss.View.dictedit({"tid":tid,"title":"编辑数据字典","renderData":v_this.renderDataBack,"pview":v_this});
        	 });
         },
            	 ...
            	 mdboss.confirm("确定要删除吗？",function(c){
            		 if(c){
            			 commonModel.save({"usercode":mdboss.usercode,"tids":delIds},
            					 {url:mdboss.api.dictdeletebatch,success:function(model,res){
                    		 if(res.rc==1){
                    			 mdboss.notifyTips("删除成功！");
                    			 v_this.renderData();
                    		 }else{
         ...
         render:function(){
        	 //字典状态
        	 mdboss.dataselect(this, "dict_state", "COMMON_STATE");
        	 this.renderData();
         },
         renderData:function(){//渲染列表
        	 ...
        	 var tmpCondition = new Array();
	            if(this.modulecode != ""){
	            	tmpCondition.push({"key":"dictcode","operate":"Like","v1":this.modulecode});
	            }
	            if(this.modulename != ""){
	            	tmpCondition.push({"key":"dictname", "operate":"Like","v1":this.modulename});
	            }
	            if(this.groupcode != ""){
	            	tmpCondition.push({"key":"groupcode", "operate":"Like","v1":this.groupcode});
	            }
	            if(this.dictState != ""){
	            	tmpCondition.push({"key":"dictstate", "operate":"EqualTo","v1":this.dictState});
	            }
             ...
             commonModel.save({ "usercode":mdboss.usercode,"pageNo":this.pageNo, "pageSize":v_this.pageSize,    // 每页10条记录
     			"queryCondition": v_this.condition,"orderBy":this.orderBy},{url:mdboss.api.sysdictlist,success:function(model,res){
     				$("#submit-loading").hide();
     				if(res.rc == 1){
