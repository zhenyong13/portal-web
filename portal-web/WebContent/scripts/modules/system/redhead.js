define(function (require, exports, module) {
     ...
     var redheadedit = require("redheadedit");
     Dialog = require("dialog");
     
     
     //使用帮助 
     exports.redheadmoduelFun=function(){
    	mdboss.fillPage(require("text!template/page/system/redhead/index.html"));
		new mdboss.View.redheadView();
     };
     
     mdboss.View.redheadView = Backbone.View.extend({
         ...
         events: {
        	 ...
        	 'change #redhead_type':"filterSearch",
         ...
         modulebatch:function(){//指修改
        	 ...
        	 if(tids.length >0){
        		 var v_this = this;
        		 redheadedit.redheadeditFunc({"tids":tids,"title":"batch","renderData":v_this.renderDataBack,"pview":v_this});
        	 }else{
        	 ...
        	 mdboss.confirm("确定要删除吗？",function(c){
        		 if(c){
        			 commonModel.save({"usercode":mdboss.usercode,"tid":tid},
        					 {url:mdboss.api.redheaddelete,success:function(model,res){
             		 	if(res.rc==1){
...
// 			var v_this = this;
        	...
        	redheadedit.redheadeditFunc({"tid":tid, "title":"view","view":true});
//        	 
         },
         moduleadd:function(){//新增 
        	 var v_this = this;
        	 redheadedit.redheadeditFunc({"title":"add","renderData":v_this.renderDataBack,"pview":v_this});
         },
         ...
         dataedit:function(e){//新增修改记录  
        	...
        	var tid=$(e.currentTarget).data("tid"); 
        	$(e.currentTarget).closest("li").addClass("selected");//
        	redheadedit.redheadeditFunc({"tid":tid,"title":"edit","renderData":v_this.renderDataBack,"pview":v_this});
         },
            	 ...
            	 mdboss.confirm("确定要删除吗？",function(c){
            		 if(c){
            			 commonModel.save({"usercode":mdboss.usercode,"tids":delIds},
            					 {url:mdboss.api.redheaddeletebatch,success:function(model,res){
                    		 if(res.rc==1){
                    			 mdboss.notifyTips("删除成功！");
                    			 v_this.renderData();
                    		 }else{
         ...
         render:function(){
        	 mdboss.dataselect(this, "redhead_type", "OA_REDDOCTYPE","");
        	 this.renderData( );
         },
         renderData:function(f){//渲染列表
        	 ...
        	 var tmpCondition = new Array();
	            ...
	            if(this.modulename){
	            	tmpCondition.push({"key":"reddocname", "operate":"Like","v1":this.modulename});
	            }
	            if(v_this.$el.find("#redhead_type").val()){
	            	tmpCondition.push({"key":"reddoctype", "operate":"EqualTo","v1":v_this.$el.find("#redhead_type").val()});
	            }
             ...
             commonModel.save({ "usercode":mdboss.usercode,"pageNo":this.pageNo, "pageSize":v_this.pageSize,    // 每页10条记录
     			"queryCondition": v_this.condition,"orderBy":this.orderBy},{url:mdboss.api.redheadlist,success:function(model,res){
     				$("#submit-loading").hide();
     				if(res.rc == 1){
