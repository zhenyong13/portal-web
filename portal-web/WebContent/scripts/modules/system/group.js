...
define(function (require, exports, module) {
     ...
     mdboss.View.systemGroupView = Backbone.View.extend({
         ...
         events: {
        	 ...
        	 'click .remove_user': 'removegroupuser',//删除群组成员
        		...
        		if(e){
        			var commonModel = new mdboss.Model.commonModel();
           			commonModel.save({"usercode":mdboss.usercode,"tid":targetusercode},
           					{url:mdboss.api.groupuserdelete, success:function(m,res){
           				if(res.rc == 1){
           					mdboss.alert("删除成功！");
         ...
         addGroupMam:function(pview, members){//添加成员 批量
        	 ...
        	 if(members && members.length>0){
        		 ...
        		 mdboss.reqsubmit({"usercode":mdboss.usercode,"records":users},
        				 mdboss.api.groupuserbatchadd, function(m, data){
        			 if(data.rc==1){
        				 mdboss.alert("成员添加成功!");
         ...
         getgroupusers:function(pview, groupcode){//添加群组成员成功后--获取群组成员
             ...
             commonModel.save({"usercode":mdboss.usercode,
            	 "queryCondition":tmpCondition,"pageNo":1, "pageSize":0},
            		 {url:mdboss.api.groupuserlist, success:function(m, data){
             	if(data.rc==1){
                     var template = Handlebars.compile(require("text!template/page/system/groupinfo.html"));
                     $('#entitySlider').html(template({"groupcode":groupcode,"groupname":pview.targetgroupname,"grouptype":grouptype,"members":data.data.rows,"baseurl":baseConfig.avatar_url})).addClass('animate-toggleR');
                     $("#user-info").mCustomScrollbar("update");
         ...
         renderUserGroup: function (e) {//群组成员信息
             ...
             var tmpCondition = new Array();
        	 tmpCondition.push({"key":"groupcode", "operate":"EqualTo","v1":groupcode});
             commonModel.save({"usercode":mdboss.usercode,"queryCondition":tmpCondition,"pageNo":1, "pageSize":0},{url:mdboss.api.groupuserlist, success:function(m, data){
             	if(data.rc==1){
                     var template = Handlebars.compile(require("text!template/page/system/groupinfo.html"));
                     $('#entitySlider').html(template({"groupcode":groupcode,"groupname":groupname,"grouptype":grouptype,"members":data.data.rows,"baseurl":baseConfig.avatar_url})).addClass('animate-toggleR');
        	 ...
        	 mdboss.confirm("确定要删除吗？",function(c){
        		 if(c){
        			 commonModel.save({"usercode":mdboss.usercode,"tid":tid},
        					 {url:mdboss.api.groupdelete,success:function(model,res){
             		 	if(res.rc==1){
         ...
         dataview:function(e){
        	 var tid=$(e.currentTarget).data("tid"); 
        	 seajs.use("groupedit", function () {
        		 new  mdboss.View.groupedit({"tid":tid, "title":"群组详情","view":true});
        	 });
         },
         moduleadd:function(){//新增
        	 var v_this = this;
        	 seajs.use("groupedit", function () {
        		 new  mdboss.View.groupedit({"title":"新增群组","renderData":v_this.renderDataBack,"pview":v_this});
        	 });
         },
         modulebatch:function(){//指修改
        	 ...
        	 if(tids.length >0){
        		 var v_this = this;
        		 seajs.use("groupedit", function () {
        			 new  mdboss.View.groupedit({"tids":tids,"title":"批量修改","renderData":v_this.renderDataBack,"pview":v_this});
        		 });
        	 }else{
         ...
         dataedit:function(e){//新增修改记录
        	...
        	var tid=$(e.currentTarget).data("tid"); 
        	 seajs.use("groupedit", function () {
        		 new  mdboss.View.groupedit({"tid":tid,"title":"编辑群组","renderData":v_this.renderDataBack,"pview":v_this});
        	 });
         },
            	 ...
            	 mdboss.confirm("确定要删除吗？",function(c){
            		 if(c){
            			 commonModel.save({"usercode":mdboss.usercode,"tids":delIds},
            					 {url:mdboss.api.groupdeletebatch,success:function(model,res){
                    		 if(res.rc==1){
                    			 mdboss.notifyTips("删除成功！");
                    			 v_this.renderData();
                    		 }else{
         ...
         renderData:function(){//渲染列表
        	 ...
        	 var tmpCondition = new Array();
	            ...
	            if(this.modulename != ""){
	            	tmpCondition.push({"key":"groupname", "operate":"Like","v1":this.modulename});
	            }
	            if(this.grouptype != ""){
             ...
             commonModel.save({ "usercode":mdboss.usercode,"pageNo":v_this.pageNo, "pageSize":v_this.pageSize,    // 每页10条记录
     			"queryCondition": v_this.condition,"orderBy":this.orderBy},{url:mdboss.api.sysgrouplist,success:function(model,res){
     				$("#submit-loading").hide();
     				if(res.rc == 1){
