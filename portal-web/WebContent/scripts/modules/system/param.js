...
define(function (require, exports, module) {
     ...
     mdboss.View.systemParam = Backbone.View.extend({
         ...
         events: {
        	 ...
        	 'change #paramtype':"searchParamtype",
        	 ...
        	 mdboss.confirm("确定要删除吗？",function(c){
        		 if(c){
        			 commonModel.save({"usercode":mdboss.usercode,"tid":tid},
        					 {url:mdboss.api.paramdelete,success:function(model,res){
             		 	if(res.rc==1){
         ...
         dataview:function(e){
        	 var tid=$(e.currentTarget).data("tid"); 
        	 seajs.use("paramedit", function () {
        		 new  mdboss.View.paramedit({"tid":tid, "title":"参数详情","view":true});
        	 });
         },
         moduleadd:function(){//新增
        	 var v_this = this;
        	 seajs.use("paramedit", function () {
        		 new  mdboss.View.paramedit({"title":"新增参数","renderData":v_this.renderDataBack,"pview":v_this});
        	 });
         },
         modulebatch:function(){//批量修改
        	 ...
        	 if(tids.length >0){
        		 var v_this = this;
        		 seajs.use("paramedit", function () {
        			 new  mdboss.View.paramedit({"tids":tids,"title":"批量修改","renderData":v_this.renderDataBack,"pview":v_this});
        		 });
        	 }else{
         ...
         dataedit:function(e){//新增修改记录
        	...
        	var tid=$(e.currentTarget).data("tid"); 
        	 seajs.use("paramedit", function () {
        		 new  mdboss.View.paramedit({"tid":tid,"title":"编辑参数","renderData":v_this.renderDataBack,"pview":v_this});
        	 });
         },
            	 ...
            	 mdboss.confirm("确定要删除吗？",function(c){
            		 if(c){
            			 commonModel.save({"usercode":mdboss.usercode,"tids":delIds},
            					 {url:mdboss.api.paramdeletebatch,success:function(model,res){
                    		 if(res.rc==1){
                    			 mdboss.notifyTips("删除成功！");
                    			 v_this.renderData();
                    		 }else{
         ...
         render:function(){
        	 mdboss.dataselect(this, "paramtype", "COMMON_PARAMTYPE");
        	 this.renderData();
         },
         renderData:function(){//渲染列表
        	 ...
        	 var tmpCondition = new Array();
	            if(this.modulecode != ""){
	            	tmpCondition.push({"key":"paramcode","operate":"Like","v1":this.modulecode});
	            }
	            if(this.modulename != ""){
	            	tmpCondition.push({"key":"paramname", "operate":"Like","v1":this.modulename});
	            }
	            if(this.paramtype != ""){
	            	tmpCondition.push({"key":"paramtype", "operate":"EqualTo","v1":this.paramtype});
	            }
             ...
             commonModel.save({ "usercode":mdboss.usercode,"pageNo":this.pageNo, "pageSize":v_this.pageSize,    // 每页10条记录
     			"queryCondition": v_this.condition,"orderBy":this.orderBy},{url:mdboss.api.sysparamlist,success:function(model,res){
     				$("#submit-loading").hide();
     				if(res.rc == 1){
