...
define(function(require, exports, module) {
	...
	mdboss.View.moduleselectView = Backbone.View.extend({
		...
		initialize : function(data) {
            ...
            }, {"title":"目录选择","width":"660px","height":"450px","icon":"iconfont icon-mdliucheng","resize":function(){
                }}).show(function(){
                	$dialog=this;
                	 mdboss.getReady({data:[{"name":"folderlist","post":{
                     	"usercode":mdboss.usercode,
                     	"type":"all",
                     	"pageNo":1,"pageSize":0,"orderBy":"parentcode"
                     }}],template:["orgs_select"]},function(result,msg){
                         if(result) {
                             var data=result.data["folderlist"];
                             var template = Handlebars.compile(result.template["orgs_select"]);
                             var html = template({"search_name":"查询目录","submit":true});
...
//                          				//回调
                  		        			...
                  		        			callbackFn(treeNode, _this.rdata.parent, _this.rdata.pview, _this.rdata.filecode);
                  		        			$dialog.close();
                          				} ,
                          			},
                          			view :{  
                             ...
                             var modulename="";
                             zNodes.push({"id":"-1","pId":"0","name":"根目录","open":true});
                                 		...
                                 		if(_this.data!=undefined){
                                 			$.each(_this.data,function(i,dept){
                                     			if(item.modulecode==dept){
                                     				zNodes.push({"id":item.foldercode,"pId":item.parentcode,"name":item.foldername,checked:true});
