define(function(require, exports, module) {
	...
	mdboss.View.newMoreView = Backbone.View.extend({
		 ...
		 renderNewsList:function(newstype){
			 ...
			 commonModel.save({
				    ...
				    "orderBy": "meetdate"
				},{url:mdboss.api.meetinfolist, success:function(m, res){
				 if(res.rc == 1){
					 //设置列表
	...
	var meetingmore = function(data) {
		mdboss.getReady({
			data:[ ],
			template : ["meetingmore"]
		}, function(result, msg) {
			//加载框框
	        mdboss.fillWrap(require("text!template/common/wrap_.html"));
	        $("#mainContainer").css("background","#fff");
			if (result) {
				mdboss.fillPage(result.template["meetingmore"] );
				 var newmoreview=new mdboss.View.newMoreView();
				 newmoreview.render();
			} else {
				 mdboss.error(msg);
			}
