define(function (require, exports, module) {
     ...
     //函数
     exports.workformFun=function(){
    	mdboss.fillPage(require("text!template/page/process/query/workform/index.html"));
		new mdboss.View.workformModule();
     };
     
     mdboss.View.workformModule = Backbone.View.extend({
         ...
         events: {
        	 ...
        	 'click .js_search_module':'searchModule',//表单搜索
         ...
         modulebatch:function(){//指修改
        	 ...
        	 if(tids.length >0){
        		 var v_this = this;
        		 seajs.use("workformedit", function () {
        			 new  mdboss.View.workformedit({"tids":tids,"title":"批量修改","renderData":v_this.renderDataBack,"pview":v_this});
        		 });
        	 }else{
         ...
         search:function(treeNode, parent, pview){//  查询父表单      	
        	 pview.pageNo = 1;
        	 ...
        	 mdboss.confirm("确定要删除吗？",function(c){
        		 if(c){
        			 commonModel.save({"usercode":mdboss.usercode,"tid":tid},
        					 {url:mdboss.api.workformdelete,success:function(model,res){
             		 	if(res.rc==1){
         ...
         dataview:function(e){
        	 var tid=$(e.currentTarget).data("tid"); 
        	 seajs.use("workformedit", function () {
        		 new  mdboss.View.workformedit({"tid":tid, "title":"表单详情","view":true});
        	 });
         },
         moduleadd:function(){//新增
        	 var v_this = this;
        	 seajs.use("workformedit", function () {
        		 new  mdboss.View.workformedit({"title":"新增表单","renderData":v_this.renderDataBack,"pview":v_this});
        	 });
         },
         dataedit:function(e){//新增修改记录
        	...
        	var tid=$(e.currentTarget).data("tid"); 
        	 seajs.use("workformedit", function () {
        		 new  mdboss.View.workformedit({"tid":tid,"title":"编辑表单","renderData":v_this.renderDataBack,"pview":v_this});
        	 });
         },
            	 ...
            	 mdboss.confirm("确定要删除吗？",function(c){
            		 if(c){
            			 commonModel.save({"usercode":mdboss.usercode,"tids":delIds},
            					 {url:mdboss.api.workformbatchdelete,success:function(model,res){
                    		 if(res.rc==1){
                    			 mdboss.notifyTips("删除成功！");
                    			 v_this.renderData();
                    		 }else{
         ...
         render:function(){
        	 mdboss.loadParentType(this,"processtype","form","");
        	 this.renderData("f");
         },
         renderData:function(f){//渲染列表
        	 ...
        	 var tmpCondition = new Array();
	            if(this.modulecode != ""){
	            	tmpCondition.push({"key":"formcode","operate":"Like","v1":this.modulecode});
	            }
	            if(this.modulename != ""){
	            	tmpCondition.push({"key":"formname", "operate":"Like","v1":this.modulename});
	            }
	            if(this.parentcode != ""){
	            	tmpCondition.push({"key":"formtype", "operate":"EqualTo","v1":this.parentcode});
	            }
             ...
             commonModel.save({ "usercode":mdboss.usercode,"pageNo":this.pageNo, "pageSize":v_this.pageSize,    // 每页10条记录
     			"queryCondition": v_this.condition,"orderBy":v_this.orderBy},{url:mdboss.api.queryworkform,success:function(model,res){
     				$("#submit-loading").hide();
     				if(res.rc == 1){
