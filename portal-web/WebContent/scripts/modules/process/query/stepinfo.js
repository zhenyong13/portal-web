define(function (require, exports, module) {
     ...
     //模块函数
     exports.stepinfoFun=function(){
    	mdboss.fillPage(require("text!template/page/process/query/stepinfo/index.html"));
		new mdboss.View.stepinfoModule();
     };
     
     mdboss.View.stepinfoModule = Backbone.View.extend({
...
//        	 'change #module_type':"search",
        	 ...
        	 'keyup #step_name':"filterSearch",//搜索
         ...
         modulebatch:function(){//指修改
        	 ...
        	 if(tids.length >0){
        		 var v_this = this;
        		 seajs.use("stepinfoedit", function () {
        			 new  mdboss.View.stepinfoedit({"tids":tids,"title":"批量修改","renderData":v_this.renderDataBack,"pview":v_this});
        		 });
        	 }else{
        	 ...
        	 mdboss.confirm("确定要删除吗？",function(c){
        		 if(c){
        			 commonModel.save({"usercode":mdboss.usercode,"tid":tid},
        					 {url:mdboss.api.stepinfodelete,success:function(model,res){
             		 	if(res.rc==1){
         ...
         dataview:function(e){
        	 var tid=$(e.currentTarget).data("tid"); 
        	 seajs.use("stepinfoedit", function () {
        		 new  mdboss.View.stepinfoedit({"tid":tid, "title":"模块详情","view":true});
        	 });
         },
         moduleadd:function(){//新增
        	 var v_this = this;
        	 seajs.use("stepinfoedit", function () {
        		 new  mdboss.View.stepinfoedit({"title":"新增模块","renderData":v_this.renderDataBack,"pview":v_this});
        	 });
         },
         dataedit:function(e){//新增修改记录
        	...
        	var tid=$(e.currentTarget).data("tid"); 
        	 seajs.use("stepinfoedit", function () {
        		 new  mdboss.View.stepinfoedit({"tid":tid,"title":"编辑模块","renderData":v_this.renderDataBack,"pview":v_this});
        	 });
         },
            	 ...
            	 mdboss.confirm("确定要删除吗？",function(c){
            		 if(c){
            			 commonModel.save({"usercode":mdboss.usercode,"tids":delIds},
            					 {url:mdboss.api.stepinfobatchdelete,success:function(model,res){
                    		 if(res.rc==1){
                    			 mdboss.notifyTips("删除成功！");
                    			 v_this.renderData();
                    		 }else{
         ...
         renderData:function(f){//渲染列表
        	 ...
        	 var tmpCondition = new Array();
	            ...
	            if(this.modulename != ""){
	            	tmpCondition.push({"key":"fromcode", "operate":"Like","v1":this.modulename});
	            }
	            if(this.parentcode != ""){
	            	tmpCondition.push({"key":"processtype", "operate":"EqualTo","v1":this.parentcode});
	            }
	            if(v_this.$el.find("#step_name").val()){
	            	tmpCondition.push({"key":"stepname", "operate":"Like","v1":v_this.$el.find("#step_name").val()});
	            }
             ...
             commonModel.save({ "usercode":mdboss.usercode,"pageNo":this.pageNo, "pageSize":v_this.pageSize,    // 每页10条记录
     			"queryCondition": v_this.condition,"orderBy":v_this.orderBy},{url:mdboss.api.stepinfolist,success:function(model,res){
     				$("#submit-loading").hide();
     				if(res.rc == 1){
