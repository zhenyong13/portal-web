define(function (require, exports, module) {
     ...
     //流程函数
     exports.workflowFun=function(){
    	mdboss.fillPage(require("text!template/page/process/query/workflow/index.html"));
		new mdboss.View.workflowModule();
     };
     
     mdboss.View.workflowModule = Backbone.View.extend({
         ...
         events: {
        	 ...
        	 'click .js_search_module':'searchModule',//流程搜索
         ...
         typesearch:function(e){
        	 this.parentcode = $("#processtype").val();
        	 this.pageNo = 1;
        	 this.renderData("f");
         },
         modulebatch:function(){//指修改
        	 ...
        	 if(tids.length >0){
        		 var v_this = this;
        		 seajs.use("workflowedit", function () {
        			 new  mdboss.View.workflowedit({"tids":tids,"title":"批量修改","renderData":v_this.renderDataBack,"pview":v_this});
        		 });
        	 }else{
         ...
         search:function(treeNode, parent, pview){//  查询父流程      	
        	 pview.pageNo = 1;
        	 ...
        	 mdboss.confirm("确定要删除吗？",function(c){
        		 if(c){
        			 commonModel.save({"usercode":mdboss.usercode,"tid":tid},
        					 {url:mdboss.api.workflowdelete,success:function(model,res){
             		 	if(res.rc==1){
         ...
         dataview:function(e){
        	 var tid=$(e.currentTarget).data("tid"); 
        	 seajs.use("workflowedit", function () {
        		 new  mdboss.View.workflowedit({"tid":tid, "title":"流程详情","view":true});
        	 });
         },
         moduleadd:function(){//新增
        	 var v_this = this;
        	 seajs.use("workflowedit", function () {
        		 new  mdboss.View.workflowedit({"title":"新增流程","renderData":v_this.renderDataBack,"pview":v_this});
        	 });
         },
         dataedit:function(e){//新增修改记录
        	...
        	var tid=$(e.currentTarget).data("tid"); 
        	 seajs.use("workflowedit", function () {
        		 new  mdboss.View.workflowedit({"tid":tid,"title":"编辑流程","renderData":v_this.renderDataBack,"pview":v_this});
        	 });
         },
            	 ...
            	 mdboss.confirm("确定要删除吗？",function(c){
            		 if(c){
            			 commonModel.save({"usercode":mdboss.usercode,"tids":delIds},
            					 {url:mdboss.api.workflowbatchdelete,success:function(model,res){
                    		 if(res.rc==1){
                    			 mdboss.notifyTips("删除成功！");
                    			 v_this.renderData();
                    		 }else{
         ...
         render:function(){
        	 mdboss.loadParentType(this,"processtype","process","");
        	 this.renderData("f");
         },
         renderData:function(f){//渲染列表
        	 ...
        	 var tmpCondition = new Array();
	            if(this.modulecode != ""){
	            	tmpCondition.push({"key":"processcode","operate":"Like","v1":this.modulecode});
	            }
	            if(this.modulename != ""){
	            	tmpCondition.push({"key":"processname", "operate":"Like","v1":this.modulename});
	            }
	            if(this.parentcode != ""){
	            	tmpCondition.push({"key":"processtype", "operate":"EqualTo","v1":this.parentcode});
	            }
             ...
             commonModel.save({ "usercode":mdboss.usercode,"pageNo":this.pageNo, "pageSize":v_this.pageSize,    // 每页10条记录
     			"queryCondition": v_this.condition,"orderBy":v_this.orderBy},{url:mdboss.api.workflowlist,success:function(model,res){
     				$("#submit-loading").hide();
     				if(res.rc == 1){
...
//	     	            	}else{
//	     	            		console.log("sdfsdf");
//	     	            		$("#module-flow-list").mCustomScrollbar("update");
//	     	            	}
