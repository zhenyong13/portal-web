define(function (require, exports, module) {
     ...
     mdboss.View.stepinfoedit = Backbone.View.extend({
         initialize:function(data){
        	 ...
        	 this.modeuledetail_html = require("text!template/page/process/query/stepinfo/view.html");
        	 this.modeuledit_html = require("text!template/page/process/query/stepinfo/edit.html");
        	 ...
        	 _this.dialog=new Dialog({
                 ...
                 fillCallback:function(){
                	 ...
                	 if(_this.tid){//编辑
	                	 ...
	                	 reqData.push( {"name" : "stepinfodetail", "post" : { "usercode":mdboss.usercode, "tid":_this.tid },cache:false });
	
	                     mdboss.getReady({ data: reqData , template: [] }, function (result, msg) {
	                         if (result) {
	                        	 var resData = result.data["stepinfodetail"];
	                        	 console.log(resData);
                 		 		if(_this.detail){//详情
                 		 			var templateA=Handlebars.compile(_this.modeuledetail_html);
                 		 			var html=templateA({"step":resData});
                 		 			dialog.$html.find(".dialog-content").html(html).fadeIn();
                 		 			 dialog.$html.find(".jscroll").jScrollPane({"autoReinitialise": true});
                 		 		}else{//编辑
                 		 			var template=Handlebars.compile(_this.modeuledit_html);
                 		 			var html=template({"step":resData});
