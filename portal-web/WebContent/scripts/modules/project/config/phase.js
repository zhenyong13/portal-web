define(function (require, exports, module) {
     ...
     var projectphaseedit = require("projectphaseedit");
     Dialog = require("dialog");
     
     
     //项目阶段
     exports.projectphaseFunc=function(){
    	mdboss.fillPage(require("text!template/page/project/config/phase/index.html"));
		new mdboss.View.projectphaseView();
     };
     
     mdboss.View.projectphaseView = Backbone.View.extend({
         ...
         modulebatch:function(){//指修改
        	 ...
        	 if(tids.length >0){
        		 var v_this = this;
        		 projectphaseedit.projectphaseeditFunc({"tids":tids,"title":"batch","renderData":v_this.renderDataBack,"pview":v_this});
        	 }else{
...
// 			var v_this = this;
        	...
        	projectphaseedit.projectphaseeditFunc({"tid":tid, "title":"view","view":true});
//        	 
         },
         moduleadd:function(){//新增 
        	 var v_this = this;
        	 projectphaseedit.projectphaseeditFunc({"title":"add","renderData":v_this.renderDataBack,"pview":v_this});
         },
         ...
         dataedit:function(e){//新增修改记录  
        	...
        	var tid=$(e.currentTarget).data("tid"); 
        	$(e.currentTarget).closest("li").addClass("selected");//
        	projectphaseedit.projectphaseeditFunc({"tid":tid,"title":"edit","renderData":v_this.renderDataBack,"pview":v_this});
         },
            	 ...
            	 mdboss.confirm("确定要删除吗？",function(c){
            		 if(c){
            			 commonModel.save({"usercode":mdboss.usercode,"tids":delIds},
            					 {url:mdboss.api.projectphasedeletebatch,success:function(model,res){
                    		 if(res.rc==1){
                    			 mdboss.notifyTips("删除成功！");
                    			 v_this.renderData();
                    		 }else{
         ...
         renderData:function(f){//渲染列表
        	 ...
        	 var tmpCondition = new Array();
	            ...
	            if(this.modulename){
	            	tmpCondition.push({"key":"phasename", "operate":"Like","v1":this.modulename});
	            }
             ...
             commonModel.save({ "usercode":mdboss.usercode,"pageNo":this.pageNo, "pageSize":v_this.pageSize,    // 每页10条记录
     			"queryCondition": v_this.condition,"orderBy":this.orderBy},{url:mdboss.api.projectphaselist,success:function(model,res){
     				$("#submit-loading").hide();
     				if(res.rc == 1){
