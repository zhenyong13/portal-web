define(function (require, exports, module) {
     ...
     //使用帮助 
     exports.roleconfigFunc=function(){
    	mdboss.fillPage(require("text!template/page/project/config/role/index.html"));
		new mdboss.View.roleconfigView();
     };
     
     mdboss.View.roleconfigView = Backbone.View.extend({
         ...
         sortingasc:function(e){//升序
        	 var col= $(e.currentTarget).data("col"); 
        	 var length = $(e.currentTarget).siblings().length;
        	 console.log("length =" + length);
        	 $(e.currentTarget).closest("li").find("span").each(function(i, item){
        		 console.log(i);
        		 if(i>=1 && i<length){
        			 console.log($(item));
        			 $(item).removeClass("sorting_asc").removeClass("sorting_desc").addClass("sorting");
        		 }
        	 });
        	 
        	 $(e.currentTarget).removeClass("sorting").addClass("sorting_asc");
        	 this.orderBy = col+ " asc";
        	 this.pageNo = 1;
        	// this.renderData();
         },
         sortingdesc:function(e){//降序
        	 var col= $(e.currentTarget).data("col"); 
        	 $(e.currentTarget).closest("li").find("span").each(function(i, item){
        		 if(i>=1 && i<length){
        			 console.log(i);
        	 ...
        	 $(e.currentTarget).removeClass("sorting").addClass("sorting_desc");
         ...
         modulebatch:function(){//指修改
        	 ...
        	 if(tids.length >0){
        		 var v_this = this;
        		 projectroleedit.projectroleeditFunc({"tids":tids,"title":"batch","renderData":v_this.renderDataBack,"pview":v_this});
        	 }else{
        	 ...
        	 mdboss.confirm("确定要删除吗？",function(c){
        		 if(c){
        			 commonModel.save({"usercode":mdboss.usercode,"tid":tid},
        					 {url:mdboss.api.projectroledelete,success:function(model,res){
             		 	if(res.rc==1){
...
// 			var v_this = this;
        	...
        	projectroleedit.projectroleeditFunc({"tid":tid, "title":"view","view":true});
//        	 
         },
         moduleadd:function(){//新增 
        	 var v_this = this;
        	 projectroleedit.projectroleeditFunc({"title":"add","renderData":v_this.renderDataBack,"pview":v_this});
         },
         ...
         dataedit:function(e){//新增修改记录  
        	...
        	var tid=$(e.currentTarget).data("tid"); 
        	$(e.currentTarget).closest("li").addClass("selected");//
        	projectroleedit.projectroleeditFunc({"tid":tid,"title":"edit","renderData":v_this.renderDataBack,"pview":v_this});
         },
            	 ...
            	 mdboss.confirm("确定要删除吗？",function(c){
            		 if(c){
            			 commonModel.save({"usercode":mdboss.usercode,"tids":delIds},
            					 {url:mdboss.api.projectroledeletebatch,success:function(model,res){
                    		 if(res.rc==1){
                    			 mdboss.notifyTips("删除成功！");
                    			 v_this.renderData();
                    		 }else{
         ...
         render:function(){
        	 mdboss.dataselect(this, "dtype", "PM_PROJECROLE","","150");
        	 mdboss.dataselect(this, "fstatus", "COMMON_STATE","","150");
//        	 mdboss.dataselect(this, "flevel", "OA_ISSUELEVEL","","150");
         ...
         renderData:function(f){//渲染列表
        	 ...
        	 var tmpCondition = new Array();
	            ...
	            if(v_this.$el.find("#dtype").val()){
	            	tmpCondition.push({"key":"roletype", "operate":"EqualTo","v1":v_this.$el.find("#dtype").val()});
	            }
             ...
             commonModel.save({ "usercode":mdboss.usercode,"pageNo":this.pageNo, "pageSize":v_this.pageSize,    // 每页10条记录
     			"queryCondition": v_this.condition,"orderBy":this.orderBy},{url:mdboss.api.projectrolelist,success:function(model,res){
     				$("#submit-loading").hide();
     				if(res.rc == 1){
