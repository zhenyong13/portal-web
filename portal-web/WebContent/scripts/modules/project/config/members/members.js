define(function (require, exports, module) {
     ...
     var membersedit = require("membersedit");
     Dialog = require("dialog");
     
     
     //项目阶段
     exports.membersFunc=function(){
    	mdboss.fillPage(require("text!template/page/project/config/members/index.html"));
		new mdboss.View.membersView();
     };
     ...
     mdboss.View.membersView = Backbone.View.extend({
         ...
         events: {
        	 ...
        	 'change #projectcode':"filterSearch",
        	 "focus #usercode":'delegateMan',//成员搜索
        	 "keyup #usercode":'delegateMan',//
        	 'change #membertype':"filterSearch",
         ...
         modulebatch:function(){//指修改
        	 ...
        	 if(tids.length >0){
        		 var v_this = this;
        		 membersedit.memberseditFunc({"tids":tids,"title":"batch","renderData":v_this.renderDataBack,"pview":v_this});
        	 }else{
        	 ...
        	 mdboss.confirm("确定要删除吗？",function(c){
        		 if(c){
        			 commonModel.save({"usercode":mdboss.usercode,"tid":tid},
        					 {url:mdboss.api.projectphasedelete,success:function(model,res){
             		 	if(res.rc==1){
...
// 			var v_this = this;
        	...
        	membersedit.memberseditFunc({"tid":tid, "title":"view","view":true});
//        	 
         },
         moduleadd:function(){//新增 
        	 var v_this = this;
        	 membersedit.memberseditFunc({"title":"add","renderData":v_this.renderDataBack,"pview":v_this});
         },
         ...
         dataedit:function(e){//新增修改记录  
        	...
        	var tid=$(e.currentTarget).data("tid"); 
        	$(e.currentTarget).closest("li").addClass("selected");//
        	membersedit.memberseditFunc({"tid":tid,"title":"edit","renderData":v_this.renderDataBack,"pview":v_this});
         },
         ...
         batchupdate:function(){//
        	 ...
        	 if(tids.length==0){
        		 ...
        		 membersedit.memberseditFunc({"tids":tids,"title":"批量修改","renderData":v_this.renderDataBack,"pview":v_this});
        	 }
         },
         checkbox:function(e){
            	 ...
            	 mdboss.confirm("确定要删除吗？",function(c){
            		 if(c){
            			 commonModel.save({"usercode":mdboss.usercode,"tids":delIds},
            					 {url:mdboss.api.membersdeletebatch,success:function(model,res){
                    		 if(res.rc==1){
                    			 mdboss.notifyTips("删除成功！");
                    			 v_this.renderData();
                    		 }else{
         ...
         render:function(){
        	 mdboss.dataselect(this, "projectcode", "#PM_PROJECROLE","","150");
        	 mdboss.dataselect(this, "membertype", "$PM_MEMBERTYPE","","150");
        	 this.renderData( );
         },
         renderData:function(f){//渲染列表
        	 ...
        	 var tmpCondition = new Array();
	            ...
	            if(v_this.$el.find("#projectcode").val()){
	            	tmpCondition.push({"key":"projectcode", "operate":"Like","v1":v_this.$el.find("#projectcode").val()});
	            }
	            if(v_this.$el.find("#usercode").val()){
	            	tmpCondition.push({"key":"usercode", "operate":"EqualTo","v1":v_this.$el.find("#usercode").val()});
	            }
	            if(v_this.$el.find("#membertype").val()){
	            	tmpCondition.push({"key":"membertype", "operate":"EqualTo","v1":v_this.$el.find("#membertype").val()});
	            }
             ...
             commonModel.save({ "usercode":mdboss.usercode,"pageNo":this.pageNo, "pageSize":v_this.pageSize,    // 每页10条记录
     			"queryCondition": v_this.condition,"orderBy":this.orderBy},{url:mdboss.api.memberslist,success:function(model,res){
     				$("#submit-loading").hide();
     				if(res.rc == 1){
